...
async def upload_resume(
    ...
    if file.content_type == "application/pdf":
        ...
        try:
            ...
            logger.info(f"PDF extracted with pdfplumber: {len(content) if content else 0} chars")
            if not content or len(content) < 50:
                content += f"\n[Full file at Cloudinary: {cloudinary_result['url']}]"
        except Exception as e:
            logger.error(f"PDF extraction failed with pdfplumber: {e}, falling back to pypdf")
            # Fallback to pypdf
            try:
                import io
                ...
                content = "\n".join(text_parts).strip()
                logger.info(f"pypdf fallback: {len(content) if content else 0} chars")
            except Exception as e2:
                logger.error(f"pypdf fallback also failed: {e2}")
                content = f"[PDF extraction failed. Full file: {cloudinary_result['url']}]"
    else:
        # For text files, try to extract content
        try:
            content = file_content.decode("utf-8", errors="ignore").strip()
            logger.info(f"Text file extracted: {len(content)} chars")
        except:
            content = None

    if not content or len(content) < 10:
        content = f"[No content extracted. Full file: {cloudinary_result['url']}]"
